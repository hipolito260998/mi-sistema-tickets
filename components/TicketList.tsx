...
export const TicketList = ({ tickets, loading }: TicketListProps) => {
  const activos = tickets.filter(t => t.status === "OPEN" || t.status === "IN_PROGRESS");
  const finalizados = tickets.filter(t => t.status === "RESOLVED" || t.status === "CLOSED");
        ...
        <div className="flex-1 overflow-y-auto pr-2 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden space-y-4">
          {loading ? (
            ...
            <div className="bg-white p-12 rounded-3xl border-2 border-dashed border-slate-100 text-center h-full flex flex-col justify-center">
              ...
              <p className="text-slate-400 font-medium">No tienes solicitudes pendientes por ahora.</p>
            </div>
          ) : (
            activos.map((ticket) => (
              <div 
              ...
              >
                ...
                <Badge className={`rounded-full px-4 py-1 font-black text-[10px] uppercase tracking-wider ${
                  ticket.status === "OPEN" ? "bg-amber-100 text-amber-700 hover:bg-amber-100" : "bg-blue-100 text-blue-700 hover:bg-blue-100"
                }`}>
